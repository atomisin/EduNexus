...
export const QuizView = ({ quiz, onComplete, isLoading, results, timeLimitMinutes = 5 }: QuizViewProps) => {
    ...
    if (!questions.length) {
        return (
            <Card className="border-0 shadow-lg text-center p-6 sm:p-12">
    ...
    if (!hasStarted) {
        return (
            <Card className="min-w-0 border-0 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                <CardHeader className="text-center pb-4">
                    ...
                    <CardTitle className="break-words text-xl sm:text-2xl">{quiz.title}</CardTitle>
        ...
        return (
            <Card className="min-w-0 border-0 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="bg-primary/5 p-4 sm:p-6 rounded-2xl border border-primary/15 relative overflow-hidden">
                        <Sparkles className="absolute top-2 right-2 w-5 h-5 text-primary/30" />
                    ...
                    <div className="space-y-4">
                        ...
                        {results.results.map((res, i) => (
                            <div key={i} className="flex min-w-0 gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
                                {res.is_correct ? (
    ...
    return (
        <Card className="border-0 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
            ...
            <CardHeader className="pb-4">
                ...
                <CardTitle className="break-words text-base sm:text-xl leading-relaxed">{quiz.title}</CardTitle>
                <Progress value={progress} className="h-1.5 mt-4" />
            </CardHeader>

            <CardContent className="py-4 sm:py-6">
                <div className="mb-8">
                    <h3 className="break-words text-base sm:text-lg font-medium text-slate-900 dark:text-slate-100 mb-5 sm:mb-6">
                    ...
                    >
                        {currentQuestion.options.map((option, idx) => {
                            ...
                            return (
                                <div key={idx} className={`flex min-w-0 items-center space-x-3 p-3 sm:p-4 rounded-xl border-2 transition-all cursor-pointer ${isSelected
                                    ? 'border-primary bg-primary/5'
                                    : 'border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700'
                                    }`} onClick={() => handleAnswerChange(label)}>
                                    <RadioGroupItem value={label} id={`option-${idx}`} className="sr-only" />
                                    <div className={`w-8 h-8 rounded-lg flex shrink-0 items-center justify-center font-bold text-sm ${isSelected ? 'bg-primary text-primary-foreground' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'
                                        }`}>
                                        {label}
                                    </div>
                                    <Label htmlFor={`option-${idx}`} className="min-w-0 flex-1 cursor-pointer break-words font-medium text-slate-700 dark:text-slate-300">
                                        {option}
                                    </Label>
                                </div>
                            );
            ...
            <CardFooter className="bg-slate-50 dark:bg-slate-900/50 p-4 sm:p-6 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
                ...
                <Button
                    ...
                    className="btn-primary w-full rounded-xl px-8 sm:w-auto"
                >
                    {isLoading ? (
